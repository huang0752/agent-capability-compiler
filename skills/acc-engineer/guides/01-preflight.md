...
- 用户明确的范围语言；未说明时按 `system_readonly_complete` 处理。
...
- 不需要生产 Secret 或生产环境。v1 候选仅使用已有 REST `GET`/`HEAD`；只有用户明确要求且本地 ACC 支持 v2 Action 时，才评估隔离测试源中的 Action，并预先确认审批与沙箱边界。
...
- 通过全部门禁后进入 Analyze。
- 若目录重叠、只读边界无法保证、需要生产凭据/生产访问，或 v2 Action 没有独立隔离沙箱和明确授权，立即停止并请求人工处理。
