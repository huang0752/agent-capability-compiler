...
2. 在 `system_complete` 中，每个 eligible excluded 路由必须引用一条顶层 `exclusion_rules` 规则，并有逐 route 的 `exclusion_decision`（独立 rationale、Evidence、capability IDs 与 replacement route IDs）。结构合法时它们是唯一权威，不再要求 `reason`；若结构不合法，仍以缺 reason 报错。Capability Plan 只保存 decision 引用，禁止维护第二份自由文本权威排除事实。
3. `operational_polling` 和 `low_business_value` 必须在 `scope.exclusion_approval` 记录用户批准原文与精确 route IDs；前端使用路由的排除同样需要精确批准。
4. ineligible、`blocked_on_evidence`、`out_of_scope`，以及 pilot/domain 模式的 excluded 路由仍要求 route-level `reason` 与 Evidence；ineligible 不进入 eligible 分母。`blocked_on_evidence` 在系统完整模式下是发布阻断，不得计为完成。
