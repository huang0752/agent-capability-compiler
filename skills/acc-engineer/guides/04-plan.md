...
3. 用户确认后生成版本化 `DomainDecision`。`system_complete` 的 completed decision 必须覆盖 Read/Create/Update/Delete/transition/execute/composite intent；Action 被 deferred、excluded 或 `blocked_on_evidence` 时领域仍未完成。只有当前领域完成后才选择下一个依赖已就绪领域；未完成领域不得批量进入实现。
...
15. 设计平台中立的 MCP tool portfolio：按业务结果组合 routes/Operations，不按 route 数机械暴露。用实际 `tools/list` 投影复核数量：每个 Read 一个工具，每个 Action 一个 `<id>.prepare`，有 Action 时另加三个共享 lifecycle 工具，并阻止保留名碰撞。仅在项目或部署显式选择人工复核 budget 时检查工具总量；核心默认不带绝对数量偏见。始终检查同 intent 且 input/output/selector 等价的重复或高度重叠、缺少 Read selector 的孤立 mutation，以及 planned/composed route 是否都可从现存 Capability 到达。门槛 warning 需要合并或保留理由，但不能靠删除工具、缩小 denominator 或把 Action 标 excluded 来消除。
