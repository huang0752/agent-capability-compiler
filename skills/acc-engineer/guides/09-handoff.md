...
2. 说明 scope mode、validation level（`offline_candidate`、`gateway_offline_verified` 或经授权的 `source_connected_verified`）、Evidence 来源、实际命令、失败/未运行项和已知限制。
...
- 发布等级必须诚实：直接 Fake Runtime 只能写 `offline_candidate`；真实 Gateway 对 Fake Source 通过才写 `gateway_offline_verified`；只有经明确授权并实际成功连接本地/测试源后才写 `source_connected_verified`。warning 不降低 `ok`，但必须进入两份风险交付物。
